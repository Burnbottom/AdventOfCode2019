def opCodeOperator(inputData):
    for opCodePos in range(0, len(inputData), 4):
            indexOne = opCodePos + 1
            indexTwo = opCodePos + 2
            indexThree = opCodePos + 3
            if inputData[opCodePos] == 99:
                return inputData
            elif inputData[opCodePos] == 1:
                inputData[inputData[indexThree]] = inputData[inputData[indexOne]] + inputData[inputData[indexTwo]]
...
def main():
    ...
    inputList[1] = 12
    inputList[2] = 2
    updatedData = opCodeOperator(inputList)
    print(f"answer to part A is: {updatedData[0]}")
