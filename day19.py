...
inputData = [int(d) for d in input.split(",")]
...
def partA(incode):
    ...
    for y in range(50):
        for x in range(50):
            comp = Computer(incode)
            pulled = next(comp.run(deque([x, y])))
            board[(x, y)] = pulled
            if pulled == 1:
                count += 1
    return (count, board)
...
def partB(incode):
    ...
    y = 4
    while True:
        if next(Computer(incode).run(deque([x, y]))):
            if next(Computer(incode).run(deque([x + 99, y - 99]))):
                print("found it", "x", x, "y", y, "tot:", x * 10000 + (y - 99))
                break
            else:
                y += 1
        else:
